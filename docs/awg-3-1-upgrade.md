...
Обновления «на месте» здесь нет: контейнер на сервере разворачивается заново. В приложении **AmneziaVPN**:

1.  Откройте **«Настройки сервера» → «AmneziaWG»**.
2.  **Удалите протокол** с сервера.
3.  **Установите его заново** — приложение развернёт актуальную версию контейнера.
...
Генератор собирает конфигурацию по группам параметров. Понимать их целиком не обязательно — значения переносятся в приложение как есть, — но полезно знать, за что отвечает каждая группа.
