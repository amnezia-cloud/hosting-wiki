...
3.  Если проверки пройдены — **смените fingerprint с `chrome` на `firefox`** на клиенте. Это помогает чаще всего: в оригинальном формате XRay (`.json`), в настройках AmneziaVPN или в ссылке `vless://` (параметр `fp=firefox`). В панели 3X-UI это поле **uTLS**: **[Панель 3X-UI](/3x-ui#inbound)**.
